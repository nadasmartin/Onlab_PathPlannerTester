...
def myError(p1x, p2x, p2y, p3x, p3y, logger, path_planner : path_planner_interface):
    ...
    #gengerate the curve
    #curve = cubic_spline_curve.CubicSplineCurve(p0, [p1x, 0], [p2x, p2y], [p3x, p3y])
    ...
    curve_length_constraint = np.abs(curve_length - 20) - 10
...
class TrackProblem(Problem):
    ...
    def __init__(self, path_planner : path_planner_interface):
        super().__init__(n_var=5, 
                         n_obj=1, 
                         n_ieq_constr=2,
                         xl=np.array([1, 3, -6, 7.5, -12]), 
                         xu=np.array([10, 14, 6, 17.5, 12]),
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(n_samples):
            ...
            #print (p2, p3, p4)
            [e, curvature_const, curve_length_const] = myError(p1x, p2x, p2y, p3x, p3y, self.logger, self.path_planner)
            error[i] = e
